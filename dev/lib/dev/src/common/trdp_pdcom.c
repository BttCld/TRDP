 ...
 *                                  other errors
 */
TRDP_ERR_T trdp_pdPut (PD_ELE_T*       pPacket,
                       ...
                       UINT32          dataSize)
{
   ...
   else if ((pData != NULL) && (dataSize != 0u))
   {
      ...
      // [BC] review condition (linearity and normalization)
      if ((marshall != NULL) && (pPacket->pktFlags & TRDP_FLAGS_MARSHALL))
      {
         ret = marshall(refCon,
                        pPacket->addr.comId,
                        (UINT8*) pData,
      ...
      {
         /* set data valid */
         pPacket->privFlags = (TRDP_PRIV_FLAGS_T)
                              (pPacket->privFlags & ~(TRDP_PRIV_FLAGS_T)TRDP_INVALID_DATA);
...
TRDP_ERR_T  trdp_pdSendQueued (TRDP_SESSION_PT appHandle)
{
   ...
   while (iterPD != NULL)
   {
       ...
       */
      if (   (timerisset(&iterPD->interval) && (!timercmp(&iterPD->timeToGo, &now, > )))
...
TRDP_ERR_T  trdp_pdReceive (TRDP_SESSION_PT appHandle, SOCKET sock)
{
  ...
  #ifdef HIGH_PERF_INDEXED
   if ((appHandle->pSlot == NULL) || (appHandle->pSlot->noOfRxEntries == 0))
   {
 ...
 */
void trdp_pdCheckPending (TRDP_APP_SESSION_T appHandle,
 ...
 */
void trdp_handleTimeout (TRDP_SESSION_PT appHandle, PD_ELE_T* pPacket)
{
   TRDP_TIME_T now;
 ...
 */
TRDP_ERR_T trdp_pdCheckListenSocks (TRDP_SESSION_PT appHandle,
                                    TRDP_FDS_T*     pRfds,
                                    INT32*          pCount)
{
   ...
   else if ((pCount != NULL) && (*pCount > 0))
   {
      ...
      for (idx = 0; idx < (UINT32) trdp_getCurrentMaxSocketCnt(TRDP_SOCK_PD); idx++)
      {
         if (   (appHandle->ifacePD[idx].sock != -1)
                /*lint !e573 signed/unsigned division in macro */
...

         {
            ...
            /*lint !e502 !e573 !e505 signed/unsigned division in macro */
...
TRDP_ERR_T  trdp_pdSend (SOCKET pdSock, PD_ELE_T* pPacket, UINT16 port)
{
   ...
   err = vos_sockSendUDP(pdSock,
                         (UINT8*)&pPacket->pFrame->frameHead, &pPacket->sendSize,
                         destIp, port);
   if (err != VOS_NO_ERR)
   {
