   ...
   if (ret == TRDP_NO_ERR)
   {
      ...
      else
      {
         ...
         else
         {
            ...
            pNewElement->pktFlags = (pktFlags == TRDP_FLAGS_DEFAULT) ?
                                    appHandle->pdDefault.flags : pktFlags;
      ...
      if ((ret == TRDP_NO_ERR) && (pNewElement != NULL))
      {
         ...
         if ((pktFlags == TRDP_FLAGS_DEFAULT) && (pfCbFunction == NULL))
         {
         ...
         *pPubHandle = (TRDP_PUB_T) pNewElement;
        ...
        #endif
         {
            ...
            {
               // [BC] start
               // avoid nesting mutex TxPD
               // ret = tlp_put(appHandle, *pPubHandle, pData, dataSize);
               // [BC] end
               ret = trdp_pdPut((PD_ELE_T*)*pPubHandle,
                                 ...
                                 TRDP_IP_ADDR_T           replyIpAddr)
{
   ...
   if ( ret == TRDP_NO_ERR)
   {
      ...
      if ((ret == TRDP_NO_ERR) && (pReqElement != NULL))
      {
         ...
         pReqElement->addr.destIpAddr = destIpAddr;
         pReqElement->addr.srcIpAddr  = srcIpAddr;
         pReqElement->addr.mcGroup    = (vos_isMulticast(destIpAddr) == 1) ?
                                        destIpAddr : VOS_INADDR_ANY;
      ...
      if (ret == TRDP_NO_ERR)
      {
         ...
         else
         {
            ...
            else
            {
               ...
               newPD->toBehavior         = (toBehavior == TRDP_TO_DEFAULT) ?
                                           appHandle->pdDefault.toBehavior : toBehavior;
               newPD->grossSize          = TRDP_MAX_PD_PACKET_SIZE;
               newPD->pUserRef           = pUserRef;
               newPD->socketIdx          = lIndex;
               newPD->privFlags         |= TRDP_INVALID_DATA;
               newPD->pktFlags           = (pktFlags == TRDP_FLAGS_DEFAULT) ?
                                           appHandle->pdDefault.flags : pktFlags;
               newPD->pfCbFunction       = (pfCbFunction == NULL) ?
   ...
   if (ret == TRDP_NO_ERR)
   {
      ...
      if (!(appHandle->option & TRDP_OPTION_BLOCK))
      {
         ...
         do
         {
           ...
           /* as long as there are messages or a timeout is received */
         } while ((err != TRDP_NODATA_ERR) && (err != TRDP_BLOCK_ERR));
      }
      ...
      /*    Check time out    */
      if (timerisset(&pElement->interval) && timercmp(&pElement->timeToGo, &now, < ))
      {
         /*    Packet is late    */
